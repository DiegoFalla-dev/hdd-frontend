...
import { SEAT_MATRICES, getMovieShowtimes } from "../data/cinemasSchedule";
...
const Butacas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (selectedCine && peliculaId && day && time && format) {
      const showtimes = getMovieShowtimes(selectedCine, peliculaId);
      const showtime = showtimes.find(s => s.date === day && s.time === time && s.format === format);
      if (showtime) {
        setSeatMatrix(showtime.seatMatrix);
      }
    }
  }, [selectedCine, peliculaId, day, time, format]);
